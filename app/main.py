...
selected_profile = st.selectbox("", sub_group)
...
st.markdown("### Desired Consumption")
...
if consumption_plot_checkbox.checkbox("View Desired Consumption"):
    if consumption_table_check_box.checkbox("View as Table"):
        ...
        servings_with_dt_fmt.index = servings_with_dt_fmt.index.strftime("%-a %b-%d %y")
        st.write(servings_with_dt_fmt)
    else:
        chart = pl.create_servings_fig(desired_servings)
        st.plotly_chart(chart)

if st.checkbox("Analyze Subscriptions"):
    try:
...
st.markdown("### Smart(er) not Harder")
